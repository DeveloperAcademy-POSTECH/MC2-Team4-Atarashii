...
struct EditCollaboraionView: View {
        ...
        VStack{
            ...
            VStack(spacing: 0) {
                HStack(spacing: 15) { // 첫 가로축
                    CollaborationKeywordButtonWithEdit(buttonColor: buttonDatas[0].buttonColor, buttonTitle: buttonDatas[0].buttonTitle , buttonSelectionTogleAction: {
                        self.buttonSelectedDatas[0].toggle()
                    })
                    .disabled(self.countTrue() >= 3 && !self.buttonSelectedDatas[0])

                    
                    CollaborationKeywordButtonWithEdit(buttonColor: buttonDatas[1].buttonColor, buttonTitle: buttonDatas[1].buttonTitle , buttonSelectionTogleAction: {self.buttonSelectedDatas[1].toggle()})
                        .disabled(self.countTrue() >= 3 && !self.buttonSelectedDatas[1])
                        .padding(.top, 36)
                    
                    CollaborationKeywordButtonWithEdit(buttonColor: buttonDatas[2].buttonColor, buttonTitle: buttonDatas[2].buttonTitle, buttonSelectionTogleAction: {self.buttonSelectedDatas[2].toggle()})
                        .disabled(self.countTrue() >= 3 && !self.buttonSelectedDatas[2])
                }
                HStack(spacing: 15){ // 두번째 가로축
                    
                    CollaborationKeywordButtonWithEdit(buttonColor: buttonDatas[3].buttonColor, buttonTitle: buttonDatas[3].buttonTitle, buttonSelectionTogleAction: {self.buttonSelectedDatas[3].toggle()})
                        .disabled(self.countTrue() >= 3 && !self.buttonSelectedDatas[3])
                        .padding(.leading, 36)
                    
                    Spacer()
                    
                    CollaborationKeywordButtonWithEdit(buttonColor: buttonDatas[4].buttonColor, buttonTitle: buttonDatas[4].buttonTitle, buttonSelectionTogleAction: {self.buttonSelectedDatas[4].toggle()})
                ...
                HStack(spacing: 15){ // 첫 가로축
                    ...
                    CollaborationKeywordButtonWithEdit(buttonColor: buttonDatas[5].buttonColor, buttonTitle: buttonDatas[5].buttonTitle, buttonSelectionTogleAction: {self.buttonSelectedDatas[5].toggle()})
                        .disabled(self.countTrue() >= 3 && !self.buttonSelectedDatas[5])
                    
                    CollaborationKeywordButtonWithEdit(buttonColor: buttonDatas[6].buttonColor, buttonTitle: buttonDatas[6].buttonTitle, buttonSelectionTogleAction: {self.buttonSelectedDatas[6].toggle()})
                        .disabled(self.countTrue() >= 3 && !self.buttonSelectedDatas[6])
                        .padding(.bottom, 36)
                    
                    CollaborationKeywordButtonWithEdit(buttonColor: buttonDatas[7].buttonColor, buttonTitle: buttonDatas[7].buttonTitle, buttonSelectionTogleAction: {self.buttonSelectedDatas[7].toggle()})
                        .disabled(self.countTrue() >= 3 && !self.buttonSelectedDatas[7])
                    
                }
                Spacer()
                cardGenerateViewsButton(title: "완료", disableCondition: self.countTrue() != 3, action: {
...
// 버튼 뷰를 리턴하는 Struct
struct CollaborationKeywordButtonWithEdit : View {
